...
// frontend display
function radiant_testimonial_render_display()
{
	global $post;
        // Set directory uri
	...
	if(is_page())
	{
		$radiant_testimonial_title = get_post_meta( $post->ID, 'ir_testimonial_title', true );
	...
	{
		$radiant_testimonial_title = cyberchimps_get_option('ir_testimonial_title');
	...
	if(  !empty($radiant_testimonial_title)  ){
	...
	<div id="radiant_testimonial_top">
		<?php if( !empty($radiant_testimonial_title) ) { ?>


				<?php if(!empty($radiant_testimonial_title)) { ?>
				<h2 class="radiant_main_title">
					<?php echo $radiant_testimonial_title; ?>
				</h2>
				<?php } ?>
      ...
      <section class="radiant_slider_text_img">
        <div id="slider2" class="flexslider">
          <ul class="slides">
			<?php	foreach( $testimonials as $testimonial) {
				...
				<li class="col-md-12">
                                    <span class="radiant_testimonial_text"><?php if(!empty($testimonial['text'])){echo $testimonial['text'];} ?></span>
					<?php if(!empty($testimonial['text']))
							{ ?>
							<hr class="after_testimonial_text"> </hr>
					<?php } ?>
					<div class="radiant_testimonial_author">
                                                <?php  if(!empty($testimonial['client'])){echo $testimonial['client'];} ?>
					</div>
					<div class="radiant_testimonial_abt_author">
	...
	<?php $slide_counters = 0; ?>
        <div id="carousel2" class="flexslider">
          <ul class="slides radiant_carousel" >
			<?php	foreach( $testimonials as $testimonial ) {
